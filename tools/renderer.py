...
from utils.utils import draw_box, draw_bbox, prob, apply
from utils.timer import Timer
from textfiller.liner import Liner
from textfiller.noiser import Noiser
from utils.font_utils import get_fonts_chars


# noinspection PyMethodMayBeStatic
class Renderer(object):
    def __init__(self, corpus, fonts, bgs, cfg, width=256, height=32,
                 clip_max_chars=False, debug=False, gpu=False, strict=False,vertical=False):
        self.corpus = corpus
        self.fonts = fonts
        self.bgs = bgs
    ...
    def gen_img(self):
        ...
        if self.vertical:
            bg = self.gen_bg(width=word_size[1] * 8, height=word_size[0] * 8)
            word_img,text_box_pnts,word_color=self.draw_vertical_text_on_bg(word, font, bg)
        else:
            bg = self.gen_bg(width=word_size[0] * 8, height=word_size[1] * 8)
            word_img, text_box_pnts, word_color = self.draw_horizontal_text_on_bg(word, font, bg)
        ...
        if self.vertical: 
            ...
            imgRotation=cv2.warpAffine(word_img,matRotation,(widthNew,heightNew),borderValue=(255,255,255))
            return imgRotation, word
    ...
    def draw_vertical_text_on_bg(self,word,font,bg):
        """
        ...
        :return
        """
        # TODO:draw the vertical text on the bg
        bg_height = bg.shape[0]
        bg_width = bg.shape[1]

        word_size = self.get_word_size(font, word)
        word_height = word_size[0]
        word_width = word_size[1]
        ...
        if apply(self.cfg.random_space):
            text_x, text_y, word_width, word_height = self.draw_text_with_random_space_vertical(draw, font, word, word_color,
                                                                                       bg_width, bg_height)
    ...
    def draw_horizontal_text_on_bg(self, word, font, bg):
        """
    ...
    def draw_text_with_random_space_vertical(self, draw, font, word, word_color, bg_width, bg_height):
        """ If random_space applied, text_x, text_y, word_width, word_height may change"""
        width = 0
        height = 0
        chars_size = []
        ...
        char_space_width = int(width * np.random.uniform(self.cfg.random_space.min, self.cfg.random_space.max))

        height += (char_space_width * (len(word) - 1))
        ...
        for i, c in enumerate(word):
            ...
            c_y += (chars_size[i][1] + char_space_width)
    ...
    def draw_text_wrapper(self, draw, text, x, y, font, text_color):
        ...
        if apply(self.cfg.text_border):
            ...
            if self.vertical:
                for i in text:
                    draw.text((x, y), i, fill=text_color, font=font)
                    y = y + font.size
            else:
    ...
    def draw_border_text(self, draw, text, x, y, font, text_color):
        ...
        if self.vertical:
            for i in text:
                # thin border
                draw.text((x - thickness, y), i, font=font, fill=border_color)
                draw.text((x + thickness, y), i, font=font, fill=border_color)
                draw.text((x, y - thickness), i, font=font, fill=border_color)
                draw.text((x, y + thickness), i, font=font, fill=border_color)

                # thicker border
                draw.text((x - thickness, y - thickness), i, font=font, fill=border_color)
                draw.text((x + thickness, y - thickness), i, font=font, fill=border_color)
                draw.text((x - thickness, y + thickness), i, font=font, fill=border_color)
                draw.text((x + thickness, y + thickness), i, font=font, fill=border_color)
